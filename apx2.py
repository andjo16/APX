...
class GraphIterator:
  ...
  def __next__(self):
    self.graph.f.seek(self.index)
    line = self.graph.f.readline()
    self.index = self.graph.f.tell()
    if line == '': 
      raise StopIteration
...
class DataFile:
  ...
  def __iter__(self):
    return GraphIterator(self) #self
  '''
  def __next__(self):
  ...
  def __contains__(self, edge):
    self.f.seek(0)
    return edge in [elm.rstrip('\n').split(' ') for elm in self.f.readlines()]
